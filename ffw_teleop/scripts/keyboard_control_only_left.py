...
class KeyboardController(Node):
    ...
    def press_and_hold(self, ctrl_key, joint_index, direction, event=None):
        self.hold_buttons.add((ctrl_key, joint_index, direction))

    def release_button(self, ctrl_key, joint_index, direction, event=None):
    ...
    def build_gui(self):
        ...
        for ctrl_key, ctrl in self.controllers.items():
            ...
            for i, joint in enumerate(ctrl['joints']):
                ...
                btn_minus.bind('<ButtonPress-1>', partial(self.press_and_hold, ctrl_key, i, -1))
                btn_minus.bind('<ButtonRelease-1>', partial(self.release_button, ctrl_key, i, -1))
                btn_plus.bind('<ButtonPress-1>', partial(self.press_and_hold, ctrl_key, i, +1))
                btn_plus.bind('<ButtonRelease-1>', partial(self.release_button, ctrl_key, i, +1))
                row += 1
